...
4. Anyone calls `settle_bet` — the program reads the oracle account on-chain, determines the winner, and pays out the vault
...
`--ignore-keys` builds against the program ID in the source (`Asn5...`) instead of a locally generated keypair, so the compiled `.so` matches the deployed program and the tests.

Uses [LiteSVM](https://github.com/LiteSVM/litesvm) — runs the compiled Solana program in-process, no validator needed.
